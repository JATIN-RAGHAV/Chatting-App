...
import { getSentFriendRequests, sentFriendRequestsReponse} from "../requests/requests";
    ...
    if (!isLoading) {
        if(users.length>0){
            if(users[0] != ''){
                return (
                <>  
                    <h1 style={{display:'flex', justifyContent:'center', marginBottom:-25}}>Sent Friend Requests</h1>
                    <div style={{display:'flex', justifyContent:'center', alignContent:'center'}}>
                        ...
                        <div style={{display:'flex', justifyContent:'center',marginTop:50,alignContent:'center',flexWrap:'wrap' }}>
                            {users.map(user => <DisplayUser key={user} username={user} handleClick={()=>{}} buttonText="Unsend Friend Request" button={false} ></DisplayUser>)}
                        </div>
                    </div>
                </>
                );
